...
namespace Global.Scenes.ScenesFlow.Runtime
{
    public class DefaultSceneLoader : ISceneLoader
    {
        ...
        public async UniTask<T> Load<T>(SceneLoadData<T> scene) where T : SceneLoadResult
        {
            ...
            {
                if (loadedScene.name != scene.Asset.Name)
                    return;
            ...
            var handle = SceneManager.LoadSceneAsync(scene.Asset.Name, LoadSceneMode.Additive);
            var task = handle.ToUniTask();
            await task;

            await UniTask.WaitUntil(() => targetScene.name == scene.Asset.Name);
